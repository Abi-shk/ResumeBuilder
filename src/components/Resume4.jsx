...
const Resume4 = () => {
  ...
  const [images, setImages] = useState({}); // Store images with resume index as key
  ...
  const downloadPDF = (resumeId) => {
    ...
    const fileInput = document.querySelector(`#resume-${resumeId} input[type="file"]`);
    if (fileInput) fileInput.style.display = 'none'; // Hide file input
    ...
    html2pdf().from(element).set(opt).toPdf().get('pdf').then((pdf) => {
      ...
      if (fileInput) fileInput.style.display = 'block'; // Show file input again
    });
  };
            ...
            <div className="flex">
              <div className="w-1/3 p-6 bg-gray-200 text-gray-800 flex flex-col items-center">
                ...
                <input
                  ...
                  className="mb-4"
                  // Temporarily show the file input for testing
                  // style={{ display: 'block' }}
                />
